...
const UNITS = [
  {
    ...
    lessons: [
      ...
      { id:57, difficulty:"advanced", title:"Reasoning Models: When AI Thinks Before It Speaks", desc:"Understand the 2025–2026 shift from chat models to reasoning models — o3, GPT-5-thinking, Claude extended thinking, DeepSeek R1. Learn what 'chain of thought' really is, when reasoning helps vs. hurts, and how to prompt these models differently from the assistants you are used to.", tags:["theory","skills","critical-thinking"], objectives:["Explain what a reasoning model is and how it differs architecturally from a standard chat model","Evaluate when extended reasoning improves accuracy and when it wastes cost, latency, or produces overthinking","Compare the reasoning approaches of o3, Claude extended thinking, Gemini 3 and DeepSeek R1","Apply prompting techniques appropriate to reasoning models — and know when to reach for a cheaper chat model instead"], resources:["r3"], prereqs:[51] },
    ]
  },
  {
    id: 1,
    ...
    colour: "#8b5cf6",
    lessons: [
      { id:9, difficulty:"intermediate", title:"Prompt Architecture",                desc:"Learn the PTFC framework — Persona, Task, Format, Context — for effective prompting, leveraging Gemini extensions and custom Gems.", tags:["practical","framework"], objectives:["Apply the PTFC framework to Gemini","Build effective prompts utilising Google Workspace extensions","Iterate prompts systematically for improved output","Critically assess when AI prompting is the right tool — and when it is not"], resources:["r17","r3"], prereqs:[1,2] },
      ...
      { id:12, difficulty:"intermediate", title:"The Revision Loop",                   desc:"Use Gemini as a Socratic feedback partner to critique and refine your drafts ethically, without offloading authorship.", tags:["practical","activity"], objectives:["Implement systematic prompt iteration","Use Gemini as a Socratic feedback critic","Critically evaluate and edit draft work based on AI suggestions","Draft clear AI disclosure notes for coursework"], resources:["r3","r17"], prereqs:[9] },
      ...
      { id:53, difficulty:"advanced", title:"RAG & AI Agents",                       desc:"Understand how Gemini Notebook uses Retrieval-Augmented Generation (RAG) to ground answers, test citation reliability, and analyse autonomous agents.", tags:["theory","practical"], objectives:["Explain the RAG pipeline from document indexing to grounded generation","Analyse how Gemini Notebook generates source-based citations and where RAG fails","Describe the ReAct agent architecture and common tool types","Evaluate the oversight challenges that arise when autonomous agents act in the world"], resources:["r1","r3"], prereqs:[9,52] },
      { id:56, difficulty:"advanced", title:"Agentic AI: Tools, Function Calling & MCP", desc:"Move from chatbots to autonomous task execution. Understand how modern agents — Claude Code, Cursor, ChatGPT agents — use function calling and the Model Context Protocol (MCP) to read files, run code, call APIs and complete multi-step work. Learn the safety architecture: approval modes, scope limits, and where agents still fail.", tags:["theory","practical","ethics"], objectives:["Explain function calling and how an LLM decides when to invoke a tool","Describe the Model Context Protocol (MCP) and why a shared standard matters for agent ecosystems","Compare the agent designs of Claude Code, Cursor, and ChatGPT agents and identify the trade-offs each makes","Apply approval modes, scope limits and least-privilege principles when deploying or using an agent","Analyse documented agent failure modes and design mitigations appropriate to the task"], resources:["r9","r3"], prereqs:[52,53] },
    ]
  },
  {
    id: 2,
  ...
  },
  {
    ...
    lessons: [
      ...
      { id:48, difficulty:"intermediate", title:"Thinking With vs. Thinking For",   desc:"Understand the school's Three-Tier AI Policy, evaluate the cognitive cost of AI summaries, and design your protocol for active learning.", tags:["reflection","discussion","critical-thinking"], objectives:["Define the Green, Amber, and Red tiers of school AI usage","Analyse the risks of cognitive offloading using the Gemini Notebook audio overview scenario","Explain why struggle builds memory and competence","Design personal protocols for maintaining cognitive independence"], resources:["r4"] },
      ...
      { id:44, difficulty:"advanced", title:"The AI Manifesto",                    desc:"Write your personal AI manifesto — a code of conduct for using Gemini and Gemini Notebook responsibly prior to the school rollout.", tags:["writing","reflection","critical-thinking"], objectives:["Synthesise course learning into a coherent personal code of conduct","Articulate specific commitments on data privacy and citation validation","Address the tensions and trade-offs of using Gemini/Gemini Notebook in school","Create a forward-looking manifesto you can genuinely defend"], resources:["r11","r4"] },
    ]
  }
];
...
/* ── Glossary ─────────────────────────────────── */
const GLOSSARY = [
  { term: 'AI Literacy', definition: 'The technical knowledge, durable skills and future-ready attitudes needed to thrive in a world influenced by AI. It enables learners to engage with, create with, manage and shape AI while critically evaluating its benefits, risks and ethics. AI literacy is more than tool use — interacting with a chatbot does not by itself build it (OECD/EU AILit Framework, 2026).', lessons: [] },
  { term: 'AILit Framework (Engage · Create · Manage · Shape)', definition: 'The OECD/European Union AI Literacy Framework (2026) for primary and secondary education. It organises AI literacy into four progressive domains: Engage with AI (be a critical, responsible participant), Create with AI (use AI as a creative partner while keeping human agency), Manage AI (divide work intentionally between humans and AI) and Shape AI (improve AI systems to reflect human values). It feeds the PISA 2029 Media & AI Literacy assessment.', lessons: [] },
  ...
  { term: 'Anthropomorphism (of AI)', definition: 'Attributing human traits — thinking, knowing, understanding, wanting — to an AI system that has none. AI produces statistically likely outputs without awareness, intent or comprehension. Anthropomorphic language quietly increases how much people trust, depend on and confide in AI, so describing AI accurately (it "computes", "predicts" or "matches patterns") is a core AI-literacy skill.', lessons: [] },
  ...
  { term: 'Cognitive Forcing Function', definition: 'A deliberate design that requires a person to think for themselves before an AI gives an answer — for example, writing your own attempt first, then revealing the AI response. Recommended in AI-in-education readiness frameworks as a research-backed antidote to passive over-reliance on AI.', lessons: [] },
  { term: 'Cognitive Offloading', definition: 'Handing mental work over to an external tool such as AI. Useful in moderation, but heavy offloading is linked to weaker critical thinking and self-regulation, especially in younger users (Gerlich, 2025).', lessons: [] },
  ...
  { term: 'Gemini', definition: 'Google\'s AI assistant — and the chatbot that comes with your school Google account (gemini.google.com). Integrates with Search, Docs and Gmail, and supports reusable custom assistants called Gems. Under the school\'s Three-Tier policy: Green for explanations and self-testing, Amber for feedback on drafts you wrote — never for writing work you submit as your own.', lessons: [9, 12, 48] },
  ...
  { term: 'LLM (Large Language Model)', definition: 'An AI model trained on vast amounts of text data that can generate, summarise, translate, and reason about language. Examples include the GPT-5 series (OpenAI), Claude Opus and Sonnet (Anthropic), Gemini 3 (Google), and Llama (Meta).', lessons: [2, 9, 15] },
  ...
  { term: 'Metacognitive Laziness', definition: 'The tendency, when AI is always available, to stop noticing what you do not understand — because the gap is filled before you feel it. Identified by Fan et al. (2025) as a risk that can weaken learners\' critical thinking and self-regulation; the antidote is deliberate, not automatic, AI use.', lessons: [] },
  { term: 'Model Card', definition: 'A short, standardised "nutrition label" for an AI system, describing its intended use, training data, performance, known limitations and ethical considerations. Reading a model card lets you judge whether a tool is fit for a task without running it — a foundational Shape-AI skill. A missing model card is itself a warning sign.', lessons: [] },
  ...
  { term: 'Gemini Notebook', definition: 'Google\'s source-grounded research notebook (formerly NotebookLM), free with a school Google account. Upload your own notes or textbook chapters and it summarises, answers questions and generates quizzes ONLY from those sources, citing the exact passage behind each answer. Built on retrieval-augmented generation (RAG).', lessons: [48, 53] },
  ...
  { term: 'Retrieval-Augmented Generation (RAG)', definition: 'A technique where the AI first retrieves relevant passages from a trusted document store, then writes its answer grounded in what it found — reducing hallucinations and enabling real citations. The architecture behind Gemini Notebook.', lessons: [53] },
  ...
  { term: 'Token', definition: 'The basic unit of text that LLMs process — roughly a word or word fragment. Models like GPT-5 and Claude process text as sequences of tokens, not as whole sentences or meanings.', lessons: [2, 9] },
...
const AI_PEOPLE = {
  ...
  "Daniela Amodei": {
    ...
    dates: "1987 –",
    bio: "Co-founder and President of Anthropic, which she founded in 2021 with her brother Dario after leaving OpenAI, where she was VP of Safety and Policy. As President she leads Anthropic\'s operations, policy and commercial work — one of the most senior women in frontier AI, and a consistent advocate for building AI companies where safety and business success reinforce each other.",
    icon: "🇺🇸"  },
  "Liang Wenfeng": {
...
const AI_TIMELINE = [
  ...
  },
  {
    ...
    summary: "Arthur Samuel builds a checkers-playing program at IBM that improves by playing games against itself — work that later led him to coin the term 'machine learning' in 1959.",
    detail: "Samuel's checkers program was remarkable because it learned to play better than its creator. It memorised positions and evaluated future moves — demonstrating that computers could acquire skills through experience rather than explicit programming. Samuel went on to coin the phrase 'machine learning' in a 1959 paper describing this capability.",
  ...
  },
  {
    ...
    detail: "NotebookLM's Audio Overview feature generated podcast-style conversations about uploaded documents. The hosts — entirely AI-generated — sounded so natural that the feature went viral on social media in October 2024. Users uploaded academic papers, legal documents, and personal notes to hear AI hosts discuss them. The tool brought AI-generated audio into mainstream awareness and sparked debate about what 'authentic' audio means in the age of generative AI. (The product was later folded into the Gemini family and rebranded Gemini Notebook.)",
    people: [],
    category: "tools",
    icon: "🎙️"
  },
  ...
  {
    year: 2026,
    title: "The GPT-5 Era — OpenAI's Flagship Models",
    summary: "OpenAI's GPT-5 family — with built-in 'thinking' modes for harder problems — becomes its flagship, combining advanced reasoning, multimodal inputs, and long-context understanding. Consumer access via ChatGPT Plus (around £16–£20/month in the UK).",
    detail: "The GPT-5 series folded the reasoning improvements of OpenAI's earlier o-series into one flagship line — the model decides how long to 'think' before answering. It handled extended, multi-step tasks across coding, analysis, and creative work. For the first time, ChatGPT's capabilities were meaningfully differentiated from free-tier alternatives, intensifying debate about equity of access in education. UK subscriptions were priced around £16–£20/month — equivalent to a school textbook per month.",
    people: ["Sam Altman"],
    category: "tools",
    icon: "🔮"
  },
  {
    year: 2026,
    title: "Gemini 3 — Google's Integrated Frontier Model",
    summary: "Google releases Gemini 3, deeply integrated into Google Workspace, Android, and Google Search — making frontier AI accessible to anyone with a Google account.",
    detail: "Gemini 3 brought frontier-level capability to users through Google's existing product ecosystem: Gmail, Docs, Drive, and Search. For education, its integration with Google Workspace for Education meant many schools — including those on Google accounts — could access state-of-the-art AI without a separate subscription. Haileybury pupils have access to Gemini through their school Google accounts. The release intensified competition with OpenAI and Anthropic and raised questions about data privacy when pupils use school-integrated AI tools.",
    people: ["Demis Hassabis", "Sundar Pichai"],
    category: "tools",
    icon: "⭐"
  },
