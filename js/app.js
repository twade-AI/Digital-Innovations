...
function exportFullProgressPDF() {
  ...
  var css = [
    ...
    'body{font-family:system-ui,-apple-system,sans-serif;background:#f8fafc;color:#0d0e24;font-size:14px}',
    /* Print action bar */
    '.pbar{background:#1a1c3a;color:#fff;padding:10px 24px;display:flex;align-items:center;justify-content:space-between;gap:16px}',
    ...
    '.pbtn:hover{background:#7a1236}',
    /* Page */
    '.page{max-width:920px;margin:24px auto;padding:0 16px 80px}',
    /* Header */
    '.rh{background:linear-gradient(135deg,#7a1236 0%,#009fe3 100%);color:#fff;border-radius:16px;padding:32px 40px;margin-bottom:24px}',
    ...
    '.card-title{font-size:.95rem;font-weight:700;color:#1a1c3a;margin-bottom:14px;display:flex;align-items:center;gap:6px}',
    /* Unit bars */
    ...
    '.badge-pill{background:#fdeff4;border:1px solid #c7d2fe;border-radius:20px;padding:7px 14px;font-size:.85rem;display:flex;align-items:center;gap:6px}',
    ...
    '.unit-hdr td{background:#fdeff4;font-weight:700;font-size:.82rem;color:#7a1236;padding:8px 10px}',
    '.lesson-num{color:#94a3b8;font-weight:700;white-space:nowrap;width:42px}',
    '.lesson-title{font-weight:600;color:#1a1c3a;max-width:220px}',
...
function printUnitCertificate(badge) {
  ...
  var html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Certificate — ' + badge.name + '</title>' +
    '<style>body{font-family:Georgia,serif;text-align:center;padding:60px;background:#fff;color:#0d0e24}' +
    ...
    '.badge-name{font-size:1.4rem;font-weight:700;color:#7a1236;margin:8px 0}.desc{font-size:.9rem;color:#475569;margin-bottom:24px}' +
...
function openAssessment() {
  ...
  var unitCards = UNITS.map(function(u, i) {
    ...
    u.lessons.forEach(function(l) {
      ...
      if (typeof QUIZ_BANK !== 'undefined' && QUIZ_BANK[l.id]) quizCount += QUIZ_BANK[l.id].length;
    });
    var mins = Math.round(quizCount * 30 / 60);
    return '<div class="au-card" onclick="startAssessment(' + i + ')">' +
      '<div class="au-icon">' + u.icon + '</div>' +
      '<div class="au-title">Unit ' + (i+1) + ': ' + u.title + '</div>' +
      '<div class="au-count">' + quizCount + ' questions · ~' + mins + ' min</div>' +
    '</div>';
  }).join('');
...
function printRevisionCard(lessonId) {
  ...
  var html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Revision Card — L' + lessonId + '</title>' +
    '<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:system-ui,sans-serif;font-size:13px;color:#0d0e24}' +
    ...
    '.quiz-box{background:#fdeff4;border-left:3px solid #9b1844;padding:8px;margin-top:10px;border-radius:4px}' +
...
function renderNewsTicker() {
  ...
  var tagColors = { policy:'#9b1844', research:'#009fe3', tools:'#22c55e', industry:'#f59e0b', ethics:'#ef4444', health:'#e6007e' };
...
function launchConfetti() {
  ...
  var particles = [];
  var colours = ['#9b1844','#009fe3','#22c55e','#f59e0b','#ef4444','#a855f7','#e6007e'];
  for (var i = 0; i < 120; i++) {
    particles.push({
