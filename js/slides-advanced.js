      ...
      choices: [
        {
          ...
          outcome: 'Jayden\'s intent is unambiguous and he should face consequences under the school\'s acceptable use policy. But answering "Jayden is responsible" explains who, not how. Security professionals ask the harder question: why was it possible for a student submitting an essay to trigger an automated email from a teacher\'s account? Jayden exploited a vulnerability that should not have existed. If a burglar walks through a door that was left unlocked, the burglar is responsible for the crime — but asking who left the door unlocked is still essential for preventing the next incident. Disciplining Jayden without redesigning the system solves this case and guarantees the next one.'
        },
        {
          text: 'The IT director — removing the human review step created the blast radius that made this attack possible and gave student-submitted content direct control over teacher email',
          outcome: 'This is the most structurally important answer. The least-privilege principle says an AI system should have only the permissions required for its specific task. ARIA\'s job was to give essay feedback — for which it needed to read student work and draft messages. It did not need to send emails directly from a teacher\'s account without confirmation. A single "Approve before sending?" step would have broken this attack entirely: Ms. Chen would have read the draft email, noticed it bore no resemblance to feedback, and deleted it. The IT director made a speed-versus-safety trade-off without consulting Ms. Chen or modelling adversarial use. That design decision — not Jayden\'s creativity — is what gave the attack its power. The same vulnerability remains for every other student who learns what Jayden knows, until the architecture changes.'
        },
        {
